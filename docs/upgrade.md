...
Indexes include *more information* and are now also *faster*. Because of this a **reindex will be necessary** when upgrading as the address and database indexes are now a part of bitcoind with three new `bzedge.conf` options:
- `-addressindex`
- `-timestampindex`
- `-spentindex`
...
- The `bzedge.conf` file in will need to be updated to include additional indexes *(see below)*.
...
```json
{
  ...
  "servicesConfig": {
    "bitcoind": {
      "spawn": {
        "datadir": "/home/<username>/.bzedge",
        "exec": "/home/<username>/bitcore-node/bin/bzedged"
      }
    }
  }
}
